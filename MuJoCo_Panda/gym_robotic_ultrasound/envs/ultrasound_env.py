...
class UltrasoundEnv(gym.Env):
    ...
    def __init__(self, position_as_action = False, controller = "VIC_Huang", log_dir = None):
...
#design teh
    ...
    def update_log(self):
        if not bool(self.log_dict): #Checks if log_dict is empty
            self.log_dict["F"] = self.controller.F_history
            ...
            self.log_dict["F"] = np.append(self.log_dict["F"], self.controller.F_history[:self.controller.timestep,:], axis = 0)
