...
def reward_bar_chart(filename, std_error = True):
    ...
    plt.xticks(x, tags, fontsize='large')
...
def run(show = True, filename = "walker_bars/checkpoints_walker.npz"):
    # reward_bar_chart(filename=filename)
    checkpoint_bars(filename)

    save_name = filename.split("/")[0] + "/walker_reward_bars.pdf"

    plt.title("Walker agent performance")
...
if __name__ == "__main__":
    ...
    for i in range(4):    
        run(filename=f"checkpoints_walker{i}.npz")