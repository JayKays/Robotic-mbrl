...
import mbrl.env.Panda_HFMC as HFMC
...
reward_fn = reward_fns.HFMC
#reward_fn = reward_fns.cartpole
# This function allows the model to know if an observation should make the episode end
term_fn = termination_fns.HFMC #no_termination#
...
for trial in range(num_trials):
    ...
    while not done:
        ...
        '''
        next_obs, reward, done, _ = common_util.step_env_and_add_to_buffer(
            env, obs, planning.RandomAgent(env), {}, replay_buffer)'''


        if done:
            print("DONE at step no. ", steps_trial)
            print("# samples stored", replay_buffer.num_stored)
        ...
        if steps_trial == trial_length:
            break
    print("iteration ", trial, " completed with total reward of ", total_reward)
