...
class VIC_Env(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(low= np.array([cfg.LOWER_F, cfg.LOWER_DELTA_Z, cfg.LOWER_VEL, \
                                        cfg.LOWER_X_ERROR, cfg.LOWER_Y_ERROR]), \
                                        high = np.array([cfg.UPPER_F, cfg.UPPER_DELTA_Z, cfg.UPPER_VEL, \
                                        cfg.UPPER_X_ERROR, cfg.UPPER_Y_ERROR]))
        ...
        self.f_d = func.generate_Fd_constant(self.max_num_it,
                                             cfg.Fd)  # func.generate_Fd_steep(self.max_num_it,cfg.T,cfg.Fd)
        ...
        self.x_d_ddot, self.x_d_dot, self.p_d = func.generate_desired_trajectory(self.robot, self.max_num_it, cfg.T,
                                                                                 self.sim, move_in_x=True)
        ...
        self.Rot_e, self.p, self.x, self.x_dot, self.x_history, self.x_dot_history, self.delta_x, self.jacobian, self.robot_inertia, self.Fz, self.F_ext, self.F_ext_2D, self.coriolis_comp = self.robot.get_VIC_states(
            self.iteration, self.time_per_iteration, self.p_d[:, self.iteration], self.goal_ori, self.x_history,
            self.x_dot_history, self.sim)
    ...
    def step(self, action):
        ...
        # adapt B and K
        xi = func.get_xi(self.x_dot, self.x_d_dot[:, self.iteration], self.x_d_ddot[:, self.iteration], self.delta_x,
                         self.x_dot_history, self.iteration, self.time_per_iteration)
        self.lam = self.lam.reshape([18, 1]) + func.get_lambda_dot(self.gamma, xi, cfg.K_v, cfg.P,
                                                                   self.f_d[:, self.iteration], self.F_ext_2D,
                                                                   self.iteration, self.time_per_iteration,
                                                                   cfg.T).reshape([18, 1])
        ...
        func.perform_torque_DeSchutter(self.robot, self.M, B_hat, K_hat, self.x_d_ddot[:, self.iteration],
                                       self.x_d_dot[:, self.iteration], self.x_dot, self.delta_x,
                                       self.p_d[:, self.iteration], self.Rot_e, self.Rot_d, self.F_ext_2D,
                                       self.jacobian, self.robot_inertia, self.coriolis_comp, self.joint_names)

        self.state = self.robot.get_6_dim_state_space(self.p_z_init, self.Fz_offset, self.f_d[2, self.iteration],
                                                      self.p_d[:, self.iteration], self.h_e_hist, self.iteration,
                                                      self.time_per_iteration)
        if cfg.ADD_NOISE:
            self.state = [self.state[0] + np.random.normal(0, abs(self.state[0] * cfg.NOISE_FRACTION)), self.state[1],
        ...
        if (self.iteration >= self.max_num_it - 1) or (np.abs(self.p_z_init - self.p[2]) > 0.05) or \
                (np.abs(self.state[3]) > 0.02) or (np.abs(self.state[4]) > 0.01) or (np.abs(self.f_d[2, self.iteration]-self.state[0]) > 5):
            done = True
            print("F:", self.state[0], " ez:", np.abs(self.p_z_init - self.p[2]), " ex:", np.abs(self.state[3]), " ey:",
                  np.abs(self.state[4]))
        ...
        return np.array(self.state)[0:5], reward, done, {}
    ...
    def reset(self):
        ...
        self.f_d = func.generate_Fd_constant(self.max_num_it,
                                             cfg.Fd)  # func.generate_Fd_steep(self.max_num_it,cfg.T,cfg.Fd)
        self.goal_ori = self.robot.endpoint_pose()[
            'orientation']  # goal orientation = current (initial) orientation [remains the same the entire duration of the run]
        self.x_d_ddot, self.x_d_dot, self.p_d = func.generate_desired_trajectory(self.robot, self.max_num_it, cfg.T,
                                                                                 self.sim, move_in_x=True)
        ...
        return np.array(self.state)[0:5]
